...
const validateProductionConfig = (nodeEnv: string, config: EnvConfig): void => {
  if (nodeEnv === 'production') {
    ...
    if (config.databaseUrl.includes('localhost')) {
      warnings.push('⚠️  WARNING: Using localhost PostgreSQL in production. Consider using managed PostgreSQL service.');
    }
...
const loadConfig = (): EnvConfig => {
  ...
  const config: EnvConfig = {
    port: parseInt(getOptionalEnvVariable('PORT', '5000'), 10),
    nodeEnv,
    databaseUrl,
